...
- **`DESPACHO_MACAU_CONFORMIDADE.md`** — ⭐ a base legal do embarque de circuito integrado para **Macau**, verificada em fonte oficial (Lei n.º 7/2003 + Lei n.º 3/2016, Tabela B do Anexo II fixada pelo Despacho 209/2021 e alterada pelos 188/2022, 208/2022 e 110/2023; emendas de e-waste da Convenção de Basileia em vigor desde 1/1/2025, entrada Y49; ECCN 3A090/4A090). **HS 8542 nunca precisou de licença** — o que travava o pacote era a nossa própria descrição, `PCB CHIPS FOR DISPOSAL`, que declarava RESÍDUO e por isso convocava o PIC. Desde 2026-08-20 o documento do gerente é **documento de DESPACHO**: sem preço para ninguém (admin inclusive), uma tabela só (categoria WTC × quantidade), caixa aduaneira com HS 8542 e um **valor declarado que NÃO é o da venda** (200–290, estável por hash da OV — `declared_value_usd` não toca em `total_usd`, porque o papel circula pelo administrativo das DUAS empresas e o preço entre elas é segredo de quem intermedia; teto sobre o real não serve, teto vaza), e um **anexo regulatório trilíngue** (EN · 繁體中文 · ES) em `_ANEXO`. A tabela traz **só o código de caixa WTC** — nem marca, nem tipo, nem capacidade, para ninguém: isso REVERTE o afrouxamento da F12 de 18/08, e `manager_document` perdeu o parâmetro `unmasked` em vez de ganhar um default. ⚠ **É declaração legal**, não interface: mudar `_ANEXO` / `SHIPMENT_DESCRIPTION` / `SHIPMENT_HS_CODE` passa pelo despachante. Detalhe em `PRECIFICACAO.md §12.19`.

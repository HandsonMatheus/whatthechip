...
python manage.py load_brands --brand piecemakers   # Marcas MIGRADAS p/ YAML (passo 4): piecemakers, gigadevice, rayson, kingston, sandisk, micron. Lê chips/knowledge/<marca>.yaml, valida c/ Pydantic, sobe catalog_version. (populate_* dessas marcas APOSENTADOS.) Dry-run padrão; --commit grava. ⚠ YAML = GRAMÁTICA (famílias+mapas); KnownParts continuam via import/fix_known_parts.

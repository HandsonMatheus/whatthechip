...
- **TABELA "GLOBAL POR ENQUANTO" É DÍVIDA COM DATA DE VENCIMENTO — e o
  teste de tenancy estava certo o tempo todo (2026-09-01):** o
  `TenancyDeclarationTests` apontava `vendas.Wallet` havia semanas e a
  leitura fácil era "falta papelada, põe na lista GLOBAL_DECLARADOS". Não
  faltava: **faltava a decisão**. A carteira era UMA linha, e a tela do
  comprador dizia, a todo comprador, *"Você paga o WhatTheChip, NUNCA o
  vendedor direto"* — frase que **contradizia a operação real da eMiner**,
  onde o Wu Quan deposita nas carteiras dela. Com um cliente só a linha
  global estava certa **por acidente**; no segundo, o comprador dele veria o
  endereço do primeiro, numa tela onde ler errado perde dinheiro de verdade.
  **Corrigido** dando `company` anulável à `Wallet` (vazio = plataforma,
  preenchido = cliente), `PlatformSharedManager` na Camada A e política de
  leitura AMPLA na Camada B (`vendas/0020`, mesmo par leitura/escrita do
  `pricing/0021`) — o teste ficou verde **por mérito**, sem entrar em
  exceção nenhuma. ⚠ **A regra do fallback:** `Wallet.for_company` NUNCA cai
  da carteira do cliente para a da plataforma nem o inverso. Fallback aqui
  manda a transferência para a parte errada em silêncio; sem endereço, a tela
  diz que não há e manda falar com o WTC — lento e certo, em vez de rápido e
  errado. ⚠ **E a frase muda junto com o endereço:** "nunca o vendedor
  direto" é aviso ANTI-GOLPE e só vale na carteira da plataforma; repeti-la
  na carteira do cliente ensinaria o comprador a desconfiar do endereço
  certo. Quem escolhe é `Wallet.is_platform`, nunca comparação com o nome do
  titular. **Regra geral: "global por enquanto" só é honesto enquanto houver
  UM cliente — e o teste de tenancy é o relógio disso. Silenciá-lo com uma
  entrada na lista é escolher não ouvir.** A migração é aditiva e **sem
  backfill de propósito**: a linha existente vira a da plataforma, e se ela
  guardar o endereço de um cliente quem sabe é o dono — adivinhar seria
  escolher para onde vai dinheiro a partir de um palpite. Travas:
  `vendas/tests_carteira_por_empresa.py` (11 testes, 6 mutações mordem,
  3 pela tela de verdade).
...
- **Receita da plataforma (2026-08-19):** o WhatTheChip retém uma **taxa de serviço** (`Company.service_fee_pct`, padrão **10%**) sobre o resultado de cada venda. O comprador paga o WTC o total CHEIO; o WTC repassa ao cliente o LÍQUIDO (`Invoice.net_usd`), registrando o `Payout`. **Duas pernas separadas**, e o cliente NUNCA vê a do comprador (valor, data, referência ou comprovante) — é segredo de mercado. A taxa congela na fatura na emissão, como o câmbio. **O repasse pode ser AUTOMÁTICO, e é decisão POR EMPRESA** (`Company.payout_on_payment`, padrão **DESLIGADO** — 2026-09-01): há arranjo comercial em que o comprador deposita direto nas contas do cliente (é o caso da eMiner — BINANCE HANDSON, TRONLINK) e o WTC nunca toca no dinheiro, só cobra a taxa por fora. Ali "comprador pagou" e "cliente recebeu" são o MESMO evento, e exigir um segundo lançamento à mão só abre a janela em que a tela do cliente diz "a receber" sobre dinheiro que já está na conta dele. Com a chave ligada, a quitação da fatura lança o `Payout` sozinha: **só na parcela que QUITA** (metade do bruto não é metade do líquido — a taxa não é proporcional a prestação nenhuma), **pelo SALDO a repassar** e não pelo líquido cru (ligar a chave numa empresa com histórico não pode dobrar o extrato), com a data do pagamento e **sem `created_by`** (ninguém digitou; quem pagou está no `Payment`). Fica na MESMA transação do pagamento e **sem `try/except`** — nesse arranjo as duas pernas são um fato só, e engolir o erro devolveria em silêncio a incoerência que isto veio matar. ⚠ O padrão TEM de ser desligado: ligado por engano, o sistema declara recebido o que ninguém transferiu. `services._repassar_automatico`; travas em `vendas/tests_repasse_automatico.py` (17 testes, 9 mutações mordem). **A MESMA chave escolhe a CARTEIRA que o comprador vê** (`Wallet.company`, 2026-09-01 — *"deve existir ambas possibilidades"*): `company` vazio = carteira do WhatTheChip, preenchido = carteira daquele cliente. Um interruptor, duas consequências, de propósito — dois permitiriam a combinação incoerente "o comprador paga o WTC e mesmo assim o sistema declara o cliente pago". `Wallet.for_company` **nunca cai de uma carteira para a outra**: sem endereço a tela DIZ que não há, porque endereço errado nesta tela é dinheiro que não volta. Detalhe em `PRECIFICACAO.md §12.19`.
- **`DESPACHO_MACAU_CONFORMIDADE.md`** — ⭐ a base legal do envio de circuito integrado para **Macau**, verificada em fonte oficial (Lei n.º 7/2003 + 3/2016, Tabela B do Anexo II, Despacho 209/2021 e as alterações 188/2022, 208/2022, 110/2023; emendas de e-waste de Basileia em vigor desde 1/1/2025, entrada Y49; ECCN 3A090/4A090) — **mais o texto completo do anexo trilíngue no §6**, guardado FORA do papel. **HS 8542 nunca precisou de licença**: o que travava o pacote era a nossa própria descrição, `PCB CHIPS FOR DISPOSAL`, que declarava RESÍDUO e por isso convocava o PIC de Basileia. ⚠ O documento que o sistema gera **NÃO é documento de despacho: é um PACKING LIST** — não é exportação, é remessa simplificada. Ele não traz código HS, não fala em aduana, venda, desembaraço ou exportação, e traz só o código de caixa WTC. **Leva um anexo curto**, em EN · 繁體中文 · ES, que declara o que a MERCADORIA é — não é resíduo (fora de Y49/A1181 de Basileia), uso final civil (não é ECCN 3A090/4A090) — e **nada sobre o trâmite**: esse é o critério, e é o que separa este anexo do de duas páginas que foi removido. O princípio que governa tudo isso, e que vale para qualquer papel do sistema: **documento que cita lei convida quem confere a ler a lei** — a fundamentação COMPLETA (licenciamento em Macau, controle de exportação) existe para RESPONDER quando perguntarem, não para viajar impressa, e mora no §6 daquele arquivo e no `ESCLARECIMENTO_LEGAL_MACAU_EN_ES_PT.docx`. O valor declarado **não é o da venda** (200–290, hash da OV; `declared_value_usd` não toca em `total_usd`, porque o papel circula pelo administrativo das DUAS empresas — e teto sobre o real não serve, teto vaza). Detalhe em `PRECIFICACAO.md §12.19`.
- **`ESCLARECIMENTO_LEGAL_MACAU_EN_ES_PT.docx`** — o documento que se ENVIA quando a transportadora ou a aduana **pergunta** (nunca junto com a caixa). Três versões completas e independentes — **inglês, espanhol e português** (o português é língua oficial de Macau) —, cada uma com natureza da mercadoria (Basileia/Y49/A1181), licenciamento de importação em Macau (Lei 7/2003 + os quatro despachos) e uso final (ECCN 3A090/4A090), mais as fontes primárias e bloco de assinatura. ⚠ É declaração assinada pela empresa: mudar passa pelo despachante. Gerado por `scripts/legal/build.js` + `content.js` (`cd scripts/legal && node build.js`) — **edite o conteúdo lá, não o .docx**, senão a próxima geração desfaz.
- **`DESIGN_SYSTEM.md`** — ⭐ o **pacote de design da v2** (`static/wtc/`): onde mora, como se linka (a ORDEM tokens → componentes → padrão não é estética: invertida a página sai sem cor, sem erro no log), o que já foi vestido e o que falta. Regras que o produto herda: `.dtab` é a tabela de TODAS as listas (e vira cartão no celular), mono em todo código/figura/valor, raio 0, sem sombra, selo em caixa alta. **A superfície do COMPRADOR não segue a regra de cópia em `v2/` do FRONTEND_V2** — aquele canary é por empresa e o comprador é de plataforma; lá se troca direto. **Leia antes de mexer em qualquer tela do parceiro.**

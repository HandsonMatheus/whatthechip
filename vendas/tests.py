...
class CompradorValorFechadoTests(TestCase):
    ...
    def test_nada_fica_ABAIXO_da_tabela_na_tela_do_comprador(self):
        ...
        # ⚠ 2026-08-27: a barra de ação `.fbar` virou o `.rhead` do pacote —
        # a faixa de cabeçalho do protótipo, que traz voltar, identidade,
        # trilho, AÇÃO e os três números numa peça só. A REGRA deste teste
        # não mudou nem um pouco: a ação fica ACIMA, e abaixo da planilha
        # não sobra controle nenhum.
        acao = html.index('rhead__act')
...
class DesignSystemNaTelaDoCompradorTests(TestCase):
    ...
    def test_a_ficha_da_compra_carrega_o_padrao_e_monta_as_quatro_pecas(self):
        ...
        # e os três números são heróis, não a fileira compacta do painel
        self.assertIn('rmx rmx--hero', barra)
        self.assertNotIn('class="kpis"', html)

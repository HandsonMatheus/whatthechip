...
YAML_PATH = "submissions/nanya_nt5cb_512m8bn_2026-08-20.yaml"
PNS = ["NT5CB512M8BN-CG", "NT5CB512M8BN-DI"]
CASE_PN = "NT5CB512M4BN"  # PN exato do debug ao vivo (M4, NAO confirmado -- ver dossie)
...
try:
    ...
    print("\n=== 6) classify() DEPOIS de aprovar (PNs M8, confirmados) ===")
    for pn in PNS:
        r = classify(pn)
    ...
    print(f"\n=== 7) classify({CASE_PN}) DE NOVO -- PN M4 exato da bancada (esperado: continua invisivel) ===")
    r = classify(CASE_PN)
    print(f"  known_exact={r.get('known_exact')} profitable={r.get('profitable')!r}")

    print("\n=== 8) classify('NT5CB512M8BN') -- mesma leitura mas com M8 em vez de M4, sem sufixo ===")
    r = classify("NT5CB512M8BN")
    print(f"  known_exact={r.get('known_exact')} profitable={r.get('profitable')!r} "
          f"(esperado: tambem invisivel, falta sufixo -CG/-DI)")

...
class Command(BaseCommand):
    ...
    def _run(self, dry, overwrite=False):
        ...
        mcp_cap = [
            ...
            #
            # ── MT29TZZZ Gen A (LPDDR3 — toda a família TZZZ é LPDDR3) ─────────────
            #
            # CORREÇÃO BUG-8 (2026-06-19): A API Micron FBGA retornava "MLC EMMC/LPDDR2"
            # para chips 8D5 (JWA60/JY941). Fontes hierarquicamente superiores contradizem:
            #   • Datasheet oficial Micron (via NXP community, PDF auditado):
            #     MT29TZZZ8D5JKEZB = "MLC e·MMC™ and Mobile LPDDR3 221-Ball MCP"
            #     com "Mobile-LPDDR3-Specific Features" e data rate até 1866 Mb/s → LPDDR3.
            #   • DigiKey: MT29TZZZ8D5BKFAH = "DRAM - LPDDR3 Memory IC ... 8Gbit (LPDDR3)"
            #   • Padrão de famílias Micron: MT29PZZZ = LPDDR2; MT29TZZZ = LPDDR3.
            #     A string "LPDDR2" da API confundia MT29PZZZ (LPDDR2, 162-ball) com
            #     MT29TZZZ (LPDDR3, 221-ball). São famílias fisicamente distintas.
            #
            ...
            #   8D4 → 40Gb  = 32Gb NAND + 8Gb RAM   ✓  "MLC EMMC/LPDDR? 40G VFBGA" LPDDR3 (familia)
            #   8D5 → 72Gb  = 64Gb NAND + 8Gb RAM   ✓  "MLC EMMC/LPDDR? 72G VFBGA" LPDDR3 (datasheet ✓)
            #   5D6 → 144Gb = 128Gb NAND + 16Gb RAM ✓  "EMCP 144G VFBGA"            (11 chips)
            #   8D6 → 136Gb = 128Gb NAND + 8Gb RAM  ✓  "MLC EMMC/LPDDR3 136G VFBGA" LPDDR3 (API ✓)
            ("4D4", "4GB",   "512MB"),  # 4→512MB RAM (4Gb)  + D4→4GB NAND (32Gb)   = 36Gb  ✓ LPDDR3
            ("8D4", "4GB",   "1GB"),    # 8→1GB RAM (8Gb)    + D4→4GB NAND (32Gb)   = 40Gb  ✓ LPDDR3
            ("8D5", "8GB",   "1GB"),    # 8→1GB RAM (8Gb)    + D5→8GB NAND (64Gb)   = 72Gb  ✓ LPDDR3
            ("5D6", "16GB",  "2GB"),    # 5→2GB RAM (16Gb)   + D6→16GB NAND (128Gb) = 144Gb ✓ LPDDR3
            ("8D6", "16GB",  "1GB"),    # 8→1GB RAM (8Gb)    + D6→16GB NAND (128Gb) = 136Gb ✓ LPDDR3
        ]
        self._bulk_map("MIC_MCP_CAP", mcp_cap, micron, dry, overwrite)

        # ── DecodeMap: geração RAM para MT29TZZZ (toda a família = LPDDR3) ──────
        #
        # MT29TZZZ é uma família LPDDR3 uniformemente — tanto Gen A (pn[8]=dígito)
        # quanto Gen B (pn[8]=letra). Não existe variante LPDDR2 dentro do prefixo TZZZ.
        # A família LPDDR2 equivalente é MT29PZZZ (prefixo diferente, 162-ball).
        #
        # BUG-8 CORRIGIDO (2026-06-19): entrada anterior mapeava '8'→LPDDR2 baseada
        # na string "MLC EMMC/LPDDR2 72G VFBGA" da API Micron FBGA — que estava ERRADA.
        # Fontes primárias (datasheet oficial + DigiKey) contradizem: chip é LPDDR3.
        #
        # Gen A (pré-2016): pn[8] = dígito (código NAND antigo) → tipo = LPDDR3
        #   Ex: MT29TZZZ[8]D5BKFAH → '8' = código NAND; RAM = LPDDR3 1GB
        #   Ex: MT29TZZZ[8]D6BKFAH → '8' = código NAND; RAM = LPDDR3 1GB
        # Gen B (2015+):  pn[8] = letra (código RAM moderno) → tipo = LPDDR3
        #   Ex: MT29TZZZ[A]D8DKKFB → 'A' = código RAM; NAND = 64GB
        #
        # ⚠ POLÍTICA: SOMENTE entradas confirmadas via datasheet oficial Micron.
        ...
        #
        # Confirmados até 2026-06-19 (datasheet Micron via NXP community + DigiKey):
    ...
    def _families(self, micron):
        return [
            ...
            # TODA a família MT29TZZZ é LPDDR3 — tanto Gen A quanto Gen B.
            # (A família LPDDR2 equivalente é MT29PZZZ, prefixo diferente.)
            #
            ...
            #   → MIC_TZZZ_GEN: pn[8]='8' → LPDDR3 (datasheet oficial Micron ✓)
            #
            # Gen B (moderna, 2015+): pn[8:11] = chave MIC_MCP_CAP (mesmo esquema MT29VZZZ)
            #   Ex: MT29TZZZAD8DKKFB → 'AD8' → 64GB NAND + 4GB LPDDR3 = 544Gb ✓
            #   → MIC_TZZZ_GEN: pn[8]='A' → LPDDR3 ✓
            #
            # Tipo RAM: decode_gen_map="MIC_TZZZ_GEN" (pos 8, 1 char)
            #   pn[8] = dígito ('4','8') → LPDDR3 (Gen A — BUG-8 corrigido 2026-06-19)
            ...
            dict(
                ...
                decode_cap_map="MIC_MCP_CAP",
                tip=(
                    "💡 Micron eMCP LPDDR3 (geração anterior ao MT29VZZZ). "
                    "Prefixo fixo MT29TZZZ (8 chars). "
                    "TODA a família é LPDDR3 — Gen A e Gen B. "
                    "RAM: pn[8] via MIC_TZZZ_GEN — dígito=LPDDR3 (Gen A), letra=LPDDR3 (Gen B). "
                    "Capacidade: pn[8:11] via MIC_MCP_CAP. "
                    "Ex Gen A: '8D5' → eMMC 5.0 8GB + LPDDR3 1GB. "
                ...
                reasoning='["MT → Micron Technology", "29 → MCP série (NAND+RAM combinados)", '
                          ...
                          '"Gen A: pn[8]=NAND code (dígito), pn[9:11]=DRAM code — chave ex: 8D5=8GB NAND+1GB LPDDR3", '
                          '"RAM: MIC_TZZZ_GEN — toda família TZZZ é LPDDR3 (BUG-8 corrigido 2026-06-19)", '

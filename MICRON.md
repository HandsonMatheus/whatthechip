...
- **BUG-9 (2026-08-20) — `import_micron_catalog.py` colapsa DDR3/DDR3L (e outras "L") no `TECH_MAP`:** o CSV oficial Micron tem `TECHNOLOGY="DDR3"` genérico + coluna separada `I/O VOLTAGE` ("1.35V"); a linha `"DDR3": ("RAM", "DDR3")` do `TECH_MAP` nunca cruza a voltagem, então todo MT41K (1,35V, deveria ser `DDR3L`) fica gravado como `DDR3` — a voltagem morre em texto livre no `notes` ("Voltage: 1.35 VOLTS"), invisível pro `chip_type`. Descoberto ao submeter D9LJJ (MT41K256M8): 25 de 40 PNs já existiam `approved` com `chip_type='DDR3'` e o próprio `notes` contradizendo (1.35V = DDR3L). Causa-raiz completa: `TECH_MAP` nunca foi migrado pro pós-endurecimento da convenção OPÇÃO 1 (29/06) — toda entrada de DRAM ali ainda usa `chip_type="RAM"` genérico (LPDDR4/4X/5/5X/5T/3/2, DDR5/4/3/2, GDDR6/5), corrigido depois só pra geração *aproximada* por `normalize_convention` (que promove `subtype`→`chip_type` cegamente, sem saber de voltagem). Não é só MT41K/DDR3L — qualquer variante "L"/baixa-voltagem coberta por esse CSV corre o mesmo risco. Lição prática: **famílias MT41x/MT47H (DDR2/DDR3/DDR3L) têm alta chance de já existir `approved` incompleto/errado** — sempre `submit_known_parts ... --commit --fill-empty`, nunca assumir "PN novo"; conflito de `chip_type` (classe "preço") resolve com `resolve_conflicts --file <arquivo> --commit` (a submissão vence, ver docstring do comando — usa esse mesmo par DDR3→DDR3L como exemplo). Correção do `TECH_MAP` em si é código, fora do escopo deste chat — sinalizado ao dono.

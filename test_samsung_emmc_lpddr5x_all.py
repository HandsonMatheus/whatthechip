#!/usr/bin/env python
"""
test_samsung_emmc_lpddr5x_all.py
=================================
Testa todos os PNs Samsung eMMC pós-2017 e LPDDR5X K3KL (extendido + large)
contra o engine WhatTheChip.

Uso (da raiz do projeto, com venv ativo):
    python test_samsung_emmc_lpddr5x_all.py
    python test_samsung_emmc_lpddr5x_all.py --only-fails
...
Fluxo recomendado antes de rodar:
    ...
    4. python manage.py import_samsung_psg --file data/psg/samsung_global_lpddr5x_k3kl_large.csv
    5. python test_samsung_emmc_lpddr5x_all.py

Chips cobertos:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  eMMC pós-2017 (KLM*):
    8GB  · 16GB · 32GB · 64GB · 128GB · 256GB
    Gerações G/R/U (pós-2017) + J/K já existentes
    ⚠ GEND-B031 = eMMC 5.0; GEND-B041 = eMMC 5.1 (mesma base PN!)

  LPDDR5X K3KL extended (2–6GB):
    2GB (9L, 6L) · 3GB (7L) · 4GB (1L, 8L) · 6GB (2L)
    ⚠ VDDQ=0.5V — NÃO confundir com LPDDR5 (0.9V)

  LPDDR5X K3KL large (6–16GB):
    6GB (DL) · 8GB (3L) · 12GB (4L) · 16GB (5L)
    ⚠ DL=novo código FM/EM gen — diferente de 2L (mesma capacidade)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
ALL_CSVS = [
    ...
    "data/psg/samsung_global_lpddr5x_k3kl_large.csv",
]
...
def run(files, only_fails):
    ...
    print(f"\n{BOLD}{'━'*72}{RESET}")
    print(f"{BOLD}  Samsung eMMC pós-2017 + LPDDR5X K3KL (extended + large) — {total} PNs{RESET}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Testa todos os PNs eMMC pós-2017 e LPDDR5X K3KL Samsung Global."
    )
    ...
    for f in ALL_CSVS:
        ...
        if not os.path.exists(path):
            print(f"{RED}Arquivo não encontrado: {path}{RED}")
            sys.exit(1)
        files.append(path)

...
YAML_PATH = "submissions/nanya_nt6tl_128t64ar_2026-08-26.yaml"
PNS = ["NT6TL128T64AR-G0", "NT6TL128T64AR-G1", "NT6TL128T64AR-G2"]
CASE_PN = "NT6TL128T64AR"  # PN exato do debug ao vivo (sem sufixo)
...
try:
    ...
    print(f"\n=== 2) classify({CASE_PN}) ANTES -- prefixo nunca visto (100% invisivel) ===")
    r = classify(CASE_PN)
    ...
    for pn in PNS:
        ...
        print(f"  {pn}: known_exact={r.get('known_exact')} chip_type={r.get('chip_type')} "
              f"capacity={r.get('capacity')!r} ram_gen={r.get('ram_gen')!r} "
              f"profitable={r.get('profitable')}")

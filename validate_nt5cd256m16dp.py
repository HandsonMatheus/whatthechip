...
YAML_PATH = "submissions/nanya_nt5cd_256m16dp_2026-08-26.yaml"
PNS = ["NT5CD256M16DP"]
CASE_PN = "NT5CD256M16DP"  # PN exato do debug ao vivo
...
try:
    ...
    print(f"\n=== 2) classify({CASE_PN}) ANTES -- reproduz o debug (prefixo nao reconhecido) ===")
    r = classify(CASE_PN)
    ...
    for pn in PNS:
        ...
        print(f"  {pn}: known_exact={r.get('known_exact')} chip_type={r.get('chip_type')} "
              ...
              f"confidence={r.get('confidence')!r}")
